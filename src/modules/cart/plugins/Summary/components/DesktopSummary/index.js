...
const Summary = (props) => {
    ...
    const Loader = () => (
        <div id="desktopSummary" className={isDesktop ? classNames(styles.container, 'hidden-mobile') : styles.container}>
            ...
            <ListItem className={classNames(styles.list, 'listSummary')}>
                <ListItemText
                    primary={(
                        <Typography variant="title" type="bold" className={classNames(styles.custom_total)}>
                            Total
                        </Typography>
                    )}
                />
                <Skeleton variant="rect" width="60%" height="30px" animation="wave" className={classNames(styles.custom_total)} />
            </ListItem>
        </div>
    );
    if (isLoader) {
    ...
    return (
        <div id="desktopSummary" className={isDesktop ? classNames(styles.container, 'hidden-mobile') : styles.container}>
            {withLabel && (
                <Typography variant="h1" type="regular" letter="capitalize" className={classNames(styles.custom_total)}>
                    {t('common:summary:title')}
                </Typography>
            )}
            {showItems ? (
                                        ...
                                        {withAction && (
                                            <div>
                                                <RemoveCircleOutlineIcon
                                                    className={classNames(styles.operator_icon)}
                                                    onClick={() => {
                                                        if (item.quantity > 1) {
            ...
            <List>
                ...
                <ListItem className={classNames(styles.list, 'listSummary')}>
                    <ListItemText
                        className={styles.labelItem}
                        primary={(
                            <Typography variant="title" type="bold" align={labelItemAlign} className={classNames(styles.custom_total)}>
                                Total
                            </Typography>
                        )}
                    />
            ...
            <div className={styles.footer}>
                {
                    !hideButton ? (
                        <Button loading={loading} disabled={disabled} className={classNames(styles.btnCheckout)} onClick={handleActionSummary}>
